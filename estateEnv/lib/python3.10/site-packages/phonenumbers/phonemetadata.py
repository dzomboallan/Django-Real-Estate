...
class NumberFormat(UnicodeMixin, ImmutableMixin):
    ...
    def __init__(
        ...
        if leading_digits_pattern is not None:
            self.leading_digits_pattern = [
                force_unicode(p) for p in leading_digits_pattern
            ]
        ...
        self.national_prefix_formatting_rule = force_unicode(
        ...
        if national_prefix_optional_when_formatting is not None:
            self.national_prefix_optional_when_formatting = bool(
        ...
        self.domestic_carrier_code_formatting_rule = force_unicode(
    ...
    def __unicode__(self):
        ...
        result = unicod("NumberFormat(pattern=%s, format=%s") % (
            rpr(self.pattern),
            rpr(self.format),
        )
        if self.leading_digits_pattern:
            result += unicod(", leading_digits_pattern=[%s]") % unicod(", ").join(
                [rpr(ld) for ld in self.leading_digits_pattern]
            )
        if self.national_prefix_formatting_rule is not None:
            result += unicod(", national_prefix_formatting_rule=%s") % rpr(
                self.national_prefix_formatting_rule
            )
        if self.national_prefix_optional_when_formatting is not None:
            result += unicod(", national_prefix_optional_when_formatting=%s") % str(
                self.national_prefix_optional_when_formatting
            )
        if self.domestic_carrier_code_formatting_rule is not None:
            result += unicod(", domestic_carrier_code_formatting_rule=%s") % rpr(
                self.domestic_carrier_code_formatting_rule
            )
        result += unicod(")")
        return result
...
class PhoneNumberDesc(UnicodeMixin, ImmutableMixin):
    ...
    def __init__(
        ...
        self.national_number_pattern = force_unicode(
            national_number_pattern
        )  # None or Unicode string holding regexp
    ...
    def __unicode__(self):
        ...
        if self.national_number_pattern is not None:
            result += unicod("%snational_number_pattern=%s") % (
                sep,
                rpr(self.national_number_pattern),
            )
            sep = unicod(", ")
        ...
        if self.possible_length:
            result += unicod("%spossible_length=%s") % (
                sep,
                tuple(self.possible_length),
            )
            sep = unicod(", ")
        if self.possible_length_local_only:
            result += unicod("%spossible_length_local_only=%s") % (
                sep,
                tuple(self.possible_length_local_only),
            )
            sep = unicod(", ")
        result += unicod(")")
        return result
...
class PhoneMetadata(UnicodeMixin, ImmutableMixin):
    ...
    def __init__(
        ...
        self.no_international_dialling = (
            no_international_dialling  # None or PhoneNumberDesc
        )
        ...
        self.international_prefix = force_unicode(
        ...
        self.preferred_international_prefix = force_unicode(
        ...
        # prefix.
        self.preferred_extn_prefix = force_unicode(
        ...
        # if that is present.
        self.national_prefix_for_parsing = force_unicode(
        ...
        self.national_prefix_transform_rule = force_unicode(
        ...
        # distinguished.
        self.same_mobile_and_fixed_line_pattern = _same_pattern(
            self.mobile, self.fixed_line
        )
        ...
        # match.
        self.leading_digits = force_unicode(
            leading_digits
        )  # None or Unicode string holding regexp
        ...
        if register:
            ...
            if self.id == REGION_CODE_FOR_NON_GEO_ENTITY:
                if self.country_code in PhoneMetadata._country_code_metadata:
                    ...
                    if self != other:
                        raise Exception(
                            "Duplicate non-geo PhoneMetadata for %s (from %s:%s)"
                            % (self.country_code, self.id, self.country_code)
                        )
                else:
                ...
                if self.id in PhoneMetadata._short_region_metadata:
                    ...
                    if self != other:
                        raise Exception(
                            "Duplicate short PhoneMetadata for %s (from %s:%s)"
                            % (self.id, self.id, self.country_code)
                        )
                else:
                ...
                if self.id in PhoneMetadata._region_metadata:
                    ...
                    if self != other:
                        raise Exception(
                            "Duplicate PhoneMetadata for %s (from %s:%s)"
                            % (self.id, self.id, self.country_code)
                        )
                else:
    ...
    def __unicode__(self):
        # Generate a string that is valid Python input for the constructor
        result = unicod(
            "PhoneMetadata(id='%s', country_code=%r, international_prefix=%s"
        ...
        if self.no_international_dialling is not None:
            result += (
                unicod(",\n    no_international_dialling=%s")
                % self.no_international_dialling
            )

        if self.preferred_international_prefix is not None:
            result += unicod(",\n    preferred_international_prefix=%s") % rpr(
        ...
        if self.preferred_extn_prefix is not None:
            result += unicod(",\n    preferred_extn_prefix=%s") % rpr(
                self.preferred_extn_prefix
            )
        if self.national_prefix_for_parsing is not None:
            result += unicod(",\n    national_prefix_for_parsing=%s") % rpr(
