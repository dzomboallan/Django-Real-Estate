...
_SLASH_SEPARATED_DATES = re.compile(
    u("(?:(?:[0-3]?\\d/[01]?\\d)|(?:[01]?\\d/[0-3]?\\d))/(?:[12]\\d)?\\d{2}")
)
...
def _verify(leniency, numobj, candidate, matcher):
    ...
    if leniency == Leniency.POSSIBLE:
        ...
        if not is_valid_number(numobj) or not _contains_only_valid_x_chars(
            numobj, candidate
        ):
            return False
...
def _verify_strict_grouping(numobj, candidate, matcher):
    if (
        not is_valid_number(numobj)
        or not _contains_only_valid_x_chars(numobj, candidate)
        or _contains_more_than_one_slash_in_national_number(numobj, candidate)
        or not _is_national_prefix_present_if_required(numobj)
    ):
        return False
    return matcher._check_number_grouping_is_valid(
        numobj, candidate, _all_number_groups_remain_grouped
    )


def _all_number_groups_remain_grouped(
    numobj, normalized_candidate, formatted_number_groups
):
    ...
    for ii, formatted_number_group in enumerate(formatted_number_groups):
        ...
        if ii == 0 and from_index < len(normalized_candidate):
            ...
            if (
                ndd_prefix_for_region(region, True) is not None
                and normalized_candidate[from_index].isdigit()
            ):
    ...
    return (
        normalized_candidate[from_index:].find(numobj.extension or U_EMPTY_STRING) != -1
    )
...
def _all_number_groups_are_exactly_present(
    numobj, normalized_candidate, formatted_number_groups
):
    """Returns True if the groups of digits found in our candidate phone number match our
    expectations.
    ...
    if (
        ...
        or candidate_groups[candidate_number_group_index].find(
    ...
    return candidate_number_group_index >= 0 and candidate_groups[
...
def _get_national_number_groups(numobj, formatting_pattern):
    ...
    return _format_nsn_using_pattern(
        nsn, formatting_pattern, PhoneNumberFormat.RFC3966
    ).split(U_DASH)
...
def _contains_more_than_one_slash_in_national_number(numobj, candidate):
    ...
    candidate_has_country_code = (
        numobj.country_code_source == CountryCodeSource.FROM_NUMBER_WITH_PLUS_SIGN
        or numobj.country_code_source == CountryCodeSource.FROM_NUMBER_WITHOUT_PLUS_SIGN
    )
    if candidate_has_country_code and normalize_digits_only(
        candidate[:first_slash_in_body_index]
...
def _is_national_prefix_present_if_required(numobj):
    ...
    if format_rule is not None and format_rule.national_prefix_formatting_rule:
        if format_rule.national_prefix_optional_when_formatting:
            ...
            return True
        if _formatting_rule_has_first_group_only(
...
class PhoneNumberMatcher(object):
    ...
    def __init__(self, text, region, leniency=Leniency.VALID, max_tries=65535):
    ...
    def _is_latin_letter(cls, letter):
        ...
        if not is_letter(letter) and Category.get(letter) != Category.NON_SPACING_MARK:
            return False
        block = Block.get(letter)
        return (
            block == Block.BASIC_LATIN
            or block == Block.LATIN_1_SUPPLEMENT
            or block == Block.LATIN_EXTENDED_A
            or block == Block.LATIN_EXTENDED_ADDITIONAL
            or block == Block.LATIN_EXTENDED_B
            or block == Block.COMBINING_DIACRITICAL_MARKS
        )
    ...
    def _is_invalid_punctuation_symbol(cls, character):
        return (
            character == U_PERCENT
            or Category.get(character) == Category.CURRENCY_SYMBOL
        )
            ...
            while group_match and self._max_tries > 0:
                if is_first_match:
                    ...
                    group = self._trim_after_first_match(
                        _UNWANTED_END_CHAR_PATTERN, candidate[: group_match.start()]
                    )
                ...
                group = self._trim_after_first_match(
                    _UNWANTED_END_CHAR_PATTERN, group_match.group(1)
                )
                match = self._parse_and_verify(group, offset + group_match.start(1))
                if match is not None:
                    return match
                self._max_tries -= 1
                group_match = possible_inner_match.search(
                    candidate, group_match.start() + 1
                )
        return None
    ...
    def _parse_and_verify(self, candidate, offset):
        ...
        try:
            ...
            if not fullmatch(_MATCHING_BRACKETS, candidate) or _PUB_PAGES.search(
                candidate
            ):
                return None
            ...
            if self.leniency >= Leniency.VALID:
                ...
                if offset > 0 and not _LEAD_PATTERN.match(candidate):
                    ...
                    # punctuation symbol
                    if self._is_invalid_punctuation_symbol(
                        previous_char
                    ) or self._is_latin_letter(previous_char):
                        return None
                ...
                if last_char_index < len(self.text):
                    ...
                    if self._is_invalid_punctuation_symbol(
                        next_char
                    ) or self._is_latin_letter(next_char):
                        return None
            ...
            for alternate_format in alternate_formats:
                if len(alternate_format.leading_digits_pattern) > 0:
                    ...
                    if not pattern.match(nsn):
                        ...
                        continue
                formatted_number_groups = _get_national_number_groups(
                    numobj, alternate_format
                )
                if checker(numobj, normalized_candidate, formatted_number_groups):
                    return True
        return False
...
class PhoneNumberMatch(UnicodeMixin):
    ...
    def __unicode__(self):
        return unicod("PhoneNumberMatch [%s,%s) %s") % (
            self.start,
            self.end,
            self.raw_string,
        )
