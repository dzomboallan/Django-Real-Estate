...
try:
    ...
    if (
        ...
        prnt(
            "Failed to import generated shortdata (but OK as during autogeneration)",
            file=sys.stderr,
        )
...
def expected_cost_for_region(short_numobj, region_dialing_from):
    ...
    if _matches_possible_number_and_national_number(
        short_number, metadata.premium_rate
    ):
        return ShortNumberCost.PREMIUM_RATE
    if _matches_possible_number_and_national_number(
        short_number, metadata.standard_rate
    ):
...
def _region_code_for_short_number_from_region_list(numobj, region_codes):
    ...
    for region_code in region_codes:
        ...
        if metadata is not None and _matches_possible_number_and_national_number(
            national_number, metadata.short_code
        ):
...
def connects_to_emergency_number(number, region_code):
    ...
    """
    return _matches_emergency_number_helper(
        number, region_code, True
    )  # Allows prefix match
...
def _matches_emergency_number_helper(number, region_code, allow_prefix_match):
    ...
    return _match_national_number(
        normalized_number, metadata.emergency, allow_prefix_match_for_region
    )
...
def is_carrier_specific(numobj):
    ...
    national_number = national_significant_number(numobj)
    metadata = PhoneMetadata.short_metadata_for_region(region_code)
    return metadata is not None and _matches_possible_number_and_national_number(
        national_number, metadata.carrier_specific
    )
...
def _matches_possible_number_and_national_number(number, number_desc):
    if number_desc is None:
        return False
    if len(number_desc.possible_length) > 0 and not (
        len(number) in number_desc.possible_length
    ):
        return False
