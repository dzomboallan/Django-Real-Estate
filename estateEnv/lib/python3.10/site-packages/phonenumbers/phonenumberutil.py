...
# area code.
_GEO_MOBILE_COUNTRIES_WITHOUT_MOBILE_AREA_CODES = frozenset((86,))  # China
...
_GEO_MOBILE_COUNTRIES = _GEO_MOBILE_COUNTRIES_WITHOUT_MOBILE_AREA_CODES | set(
    (52, 54, 55, 62)  # Mexico  # Argentina  # Brazil
...
_DIALLABLE_CHAR_MAPPINGS = dict(
    {_PLUS_SIGN: _PLUS_SIGN, u("*"): u("*"), u("#"): u("#")}, **_ASCII_DIGITS_MAP
)

# Separate map of all symbols that we wish to retain when formatting alpha
# numbers. This includes digits, ASCII letters and number grouping symbols
# such as "-" and " ".
_ALL_PLUS_NUMBER_GROUPING_SYMBOLS = dict(
    {
        u("-"): u("-"),  # Add grouping symbols.
        u("\uFF0D"): u("-"),
        u("\u2010"): u("-"),
        u("\u2011"): u("-"),
        u("\u2012"): u("-"),
    ...
    **dict(
        [(_c.lower(), _c) for _c in _ALPHA_MAPPINGS.keys()]
...
_VALID_PUNCTUATION = u("-x\u2010-\u2015\u2212\u30FC\uFF0D-\uFF0F ") + u(
    "\u00A0\u00AD\u200B\u2060\u3000()\uFF08\uFF09\uFF3B\uFF3D.\\[\\]/~\u2053\u223C\uFF5E"
)

_DIGITS = unicod(
    "\\d"
)  # Java "\\p{Nd}", so need "(?u)" or re.UNICODE wherever this is used
# We accept alpha characters in phone numbers, ASCII only, upper and lower
# case.
_VALID_ALPHA = U_EMPTY_STRING.join(_ALPHA_MAPPINGS.keys()) + U_EMPTY_STRING.join(
    [_k.lower() for _k in _ALPHA_MAPPINGS.keys()]
)
_PLUS_CHARS = u("+\uFF0B")
...
def _create_extn_pattern(for_parsing):
    ...
    explicit_extn = (
        ...
        + _extn_digits(ext_limit_after_explicit_label)
    ...
    ambiguous_extn = (
        ...
        + possible_chars_after_ext_label
        + _extn_digits(ext_limit_after_ambiguous_char)
        + optional_extn_suffix
    )
    american_style_extn_with_suffix = (
        ambiguous_separator + _extn_digits(ext_limit_when_not_sure) + u("#")
    )
    ...
    if for_parsing:
        ...
        auto_dialling_extn = (
            ...
            + possible_chars_after_ext_label
            + _extn_digits(ext_limit_after_likely_label)
            + optional_extn_suffix
        )
        only_commas_extn = (
            possible_separators_number_ext_label_no_comma
        ...
        return (
            extension_pattern + u("|") + auto_dialling_extn + u("|") + only_commas_extn
        )
...
_RFC3966_GLOBAL_NUMBER_DIGITS = (
    "^\\"
    + _PLUS_SIGN
    + _RFC3966_PHONE_DIGIT
    + "*"
    + _DIGITS
    + _RFC3966_PHONE_DIGIT
    + "*$"
...
_RFC3966_DOMAINNAME = (
    "^(" + _RFC3966_DOMAINLABEL + "\\.)*" + _RFC3966_TOPLABEL + "\\.?$"
)
...
_EXTN_PATTERN = re.compile(
    u("(?:") + _EXTN_PATTERNS_FOR_PARSING + u(")$"), _REGEX_FLAGS
)

# We append optionally the extension pattern to the end here, as a valid phone
# number may have an extension prefix appended, followed by 1 or more digits.
_VALID_PHONE_NUMBER_PATTERN = re.compile(
    _VALID_PHONE_NUMBER + u("(?:") + _EXTN_PATTERNS_FOR_PARSING + u(")?"), _REGEX_FLAGS
)
...
def _desc_has_data(desc):
    ...
    return (
        (desc.example_number is not None)
        or _desc_has_possible_number_data(desc)
        or (desc.national_number_pattern is not None)
    )
...
def _formatting_rule_has_first_group_only(national_prefix_formatting_rule):
    ...
    if national_prefix_formatting_rule is None:
        return True
    return bool(
        fullmatch(_FIRST_GROUP_ONLY_PREFIX_PATTERN, national_prefix_formatting_rule)
    )
...
def is_number_type_geographical(num_type, country_code):
    ...
    """
    return (
        num_type == PhoneNumberType.FIXED_LINE
        or num_type == PhoneNumberType.FIXED_LINE_OR_MOBILE
        or (
            (country_code in _GEO_MOBILE_COUNTRIES)
            and num_type == PhoneNumberType.MOBILE
        )
    )
...
def format_number(numobj, num_format):
    ...
    if num_format == PhoneNumberFormat.E164:
        ...
        return _prefix_number_with_country_calling_code(
            country_calling_code, num_format, nsn
        )
    ...
    metadata = PhoneMetadata.metadata_for_region_or_calling_code(
        country_calling_code, region_code.upper()
    )
    formatted_number = _format_nsn(nsn, metadata, num_format)
    formatted_number = _maybe_append_formatted_extension(
        numobj, metadata, num_format, formatted_number
    )
    return _prefix_number_with_country_calling_code(
        country_calling_code, num_format, formatted_number
    )
    ...
    if formatting_pattern is None:
        ...
        if np_formatting_rule:
            ...
            if national_prefix:
                ...
                # group (\1) matcher.
                np_formatting_rule = np_formatting_rule.replace(
                    _NP_STRING, national_prefix
                )
                np_formatting_rule = np_formatting_rule.replace(
                    _FG_STRING, unicod("\\1")
                )
        ...
        formatted_number = _format_nsn_using_pattern(
            nsn, num_format_copy, number_format
        )
    formatted_number = _maybe_append_formatted_extension(
        numobj, metadata, number_format, formatted_number
    )
    formatted_number = _prefix_number_with_country_calling_code(
        country_code, number_format, formatted_number
    )
...
def format_national_number_with_carrier_code(numobj, carrier_code):
    ...
    formatted_number = _format_nsn(
        nsn, metadata, PhoneNumberFormat.NATIONAL, carrier_code
    )
    formatted_number = _maybe_append_formatted_extension(
        numobj, metadata, PhoneNumberFormat.NATIONAL, formatted_number
    )
    formatted_number = _prefix_number_with_country_calling_code(
        country_code, PhoneNumberFormat.NATIONAL, formatted_number
    )
    return formatted_number
...
def format_national_number_with_preferred_carrier_code(numobj, fallback_carrier_code):
    ...
    if (
        numobj.preferred_domestic_carrier_code is not None
        and len(numobj.preferred_domestic_carrier_code) > 0
    ):
...
def format_number_for_mobile_dialing(numobj, region_calling_from, with_formatting):
    ...
    if region_calling_from == region_code:
        is_fixed_line_or_mobile = (
            ...
            or (numobj_type == PhoneNumberType.MOBILE)
            or (numobj_type == PhoneNumberType.FIXED_LINE_OR_MOBILE)
        )
        ...
        if region_code == "BR" and is_fixed_line_or_mobile:
            ...
            if (
                numobj_no_ext.preferred_domestic_carrier_code is not None
                and len(numobj_no_ext.preferred_domestic_carrier_code) > 0
            ):
                formatted_number = format_national_number_with_preferred_carrier_code(
                    numobj_no_ext, ""
                )
            else:
            ...
            )  # due to _has_valid_country_calling_code() check
            if (
                can_be_internationally_dialled(numobj_no_ext)
                and _test_number_length(
                    national_significant_number(numobj_no_ext), metadata
                )
                != ValidationResult.TOO_SHORT
            ):
                formatted_number = format_number(
                    numobj_no_ext, PhoneNumberFormat.INTERNATIONAL
                )
            else:
                formatted_number = format_number(
                    numobj_no_ext, PhoneNumberFormat.NATIONAL
                )
        else:
            ...
            if (
                region_code == REGION_CODE_FOR_NON_GEO_ENTITY
                or (
                    (
                        region_code == unicod("MX")
                        or region_code == unicod("CL")
                        or region_code == unicod("UZ")
                    )
                    and is_fixed_line_or_mobile
                )
            ) and can_be_internationally_dialled(numobj_no_ext):
...
def format_out_of_country_calling_number(numobj, region_calling_from):
    ...
    metadata_for_region_calling_from = (
        PhoneMetadata.metadata_for_region_or_calling_code(
            country_code, region_calling_from.upper()
        )
    )
    ...
    region_code = region_code_for_country_code(country_code)
    # Metadata cannot be None because the country calling code is valid.
    metadata_for_region = PhoneMetadata.metadata_for_region_or_calling_code(
        country_code, region_code.upper()
    )
    formatted_national_number = _format_nsn(
        nsn, metadata_for_region, PhoneNumberFormat.INTERNATIONAL
    )
    formatted_number = _maybe_append_formatted_extension(
        numobj,
        metadata_for_region,
    ...
    if len(i18n_prefix_for_formatting) > 0:
        ...
        formatted_number = _prefix_number_with_country_calling_code(
            country_code, PhoneNumberFormat.INTERNATIONAL, formatted_number
        )
    return formatted_number
...
def _format_original_allow_mods(numobj, region_calling_from):
    if numobj.country_code_source == CountryCodeSource.FROM_NUMBER_WITH_PLUS_SIGN:
        ...
        if _raw_input_contains_national_prefix(
            numobj.raw_input, national_prefix, region_code
        ):
        ...
        format_rule = _choose_formatting_pattern_for_number(
        ...
        if index_of_first_group <= 0:
            return national_format
        candidate_national_prefix_rule = candidate_national_prefix_rule[
...
def _has_formatting_pattern_for_number(numobj):
    ...
    metadata = PhoneMetadata.metadata_for_region_or_calling_code(
        country_code, phone_number_region
    )
    if metadata is None:
        return False
    national_number = national_significant_number(numobj)
...
def format_out_of_country_keeping_alpha_chars(numobj, region_calling_from):
    ...
    num_raw_input = _normalize_helper(
        num_raw_input, _ALL_PLUS_NUMBER_GROUPING_SYMBOLS, True
    )
    ...
    metadata_for_region_calling_from = PhoneMetadata.metadata_for_region(
        region_calling_from.upper(), None
    )
    if country_code == _NANPA_COUNTRY_CODE:
        if is_nanpa_country(region_calling_from):
            return unicod(country_code) + U_SPACE + num_raw_input
    elif (
        metadata_for_region_calling_from is not None
        and country_code == country_code_for_region(region_calling_from)
    ):
        formatting_pattern = _choose_formatting_pattern_for_number(
            metadata_for_region_calling_from.number_format, national_number
        )
        if formatting_pattern is None:
        ...
        return _format_nsn_using_pattern(
            num_raw_input, new_format, PhoneNumberFormat.NATIONAL
        )
    i18n_prefix_for_formatting = U_EMPTY_STRING
    ...
    formatted_number = _maybe_append_formatted_extension(
        numobj, metadata_for_region, PhoneNumberFormat.INTERNATIONAL, num_raw_input
    )
...
def _prefix_number_with_country_calling_code(
    country_code, num_format, formatted_number
):
...
def _format_nsn(number, metadata, num_format, carrier_code=None):
    ...
    if len(intl_number_formats) == 0 or num_format == PhoneNumberFormat.NATIONAL:
    ...
    if formatting_pattern is None:
        ...
        return _format_nsn_using_pattern(
            number, formatting_pattern, num_format, carrier_code
        )
...
def _format_nsn_using_pattern(
    national_number, formatting_pattern, number_format, carrier_code=None
):
    ...
    if (
        ...
        and carrier_code
        and formatting_pattern.domestic_carrier_code_formatting_rule
    ):
        ...
        # appropriate way.
        number_format_rule = re.sub(
            _FIRST_GROUP_PATTERN, cc_format_rule, number_format_rule, count=1
        )
        ...
        if (
            number_format == PhoneNumberFormat.NATIONAL
            and national_prefix_formatting_rule
        ):
            first_group_rule = re.sub(
            ...
            formatted_national_number = re.sub(
                m_re, number_format_rule, national_number
            )
    ...
    if number_format == PhoneNumberFormat.RFC3966:
        ...
        if m:
            formatted_national_number = re.sub(
                _SEPARATOR_PATTERN, U_EMPTY_STRING, formatted_national_number, count=1
            )
        # Replace the rest with a dash between each number group
        formatted_national_number = re.sub(
            _SEPARATOR_PATTERN, U_DASH, formatted_national_number
        )
...
def _example_number_anywhere_for_type(num_type):
    ...
    for country_calling_code in COUNTRY_CODES_FOR_NON_GEO_REGIONS:
        ...
        if desc is not None and desc.example_number is not None:
            try:
                return parse(
                    _PLUS_SIGN + unicod(country_calling_code) + desc.example_number,
                    UNKNOWN_REGION,
                )
            except NumberParseException:  # pragma no cover
                pass
...
def number_type(numobj):
    ...
    metadata = PhoneMetadata.metadata_for_region_or_calling_code(
        numobj.country_code, region_code
    )
    if metadata is None:
...
def _number_type_helper(national_number, metadata):
    ...
    if not metadata.same_mobile_and_fixed_line_pattern and _is_number_matching_desc(
        national_number, metadata.mobile
    ):
...
def is_valid_number_for_region(numobj, region_code):
    ...
    if metadata is None or (
        region_code != REGION_CODE_FOR_NON_GEO_ENTITY
        and country_code != country_code_for_valid_region(region_code)
    ):
...
def is_possible_number(numobj):
    ...
    return (
        result == ValidationResult.IS_POSSIBLE
        or result == ValidationResult.IS_POSSIBLE_LOCAL_ONLY
    )
...
def _test_number_length(national_number, metadata, numtype=PhoneNumberType.UNKNOWN):
    ...
    if desc_for_type is None:
        ...
        if (
            len(possible_lengths) == 0
        ):  # pragma no cover: Python sub-descs all have possible_length
            possible_lengths = metadata.general_desc.possible_length
        local_lengths = desc_for_type.possible_length_local_only

    if numtype == PhoneNumberType.FIXED_LINE_OR_MOBILE:
        if not _desc_has_possible_number_data(
            _number_desc_by_type(metadata, PhoneNumberType.FIXED_LINE)
        ):
            # The rare case has been encountered where no fixedLine data is available (true for some
            # non-geographical entities), so we just check mobile.
            return _test_number_length(
                national_number, metadata, PhoneNumberType.MOBILE
            )
        else:
            ...
            if _desc_has_possible_number_data(mobile_desc):
                ...
                if (
                    len(mobile_desc.possible_length) == 0
                ):  # pragma no cover: Python sub-descs all have possible_length
                    possible_lengths += metadata.general_desc.possible_length
                else:
...
def _maybe_extract_country_code(number, metadata, keep_raw_input, numobj):
    ...
    country_code_source, full_number = _maybe_strip_i18n_prefix_and_normalize(
    ...
    if country_code_source != CountryCodeSource.FROM_DEFAULT_COUNTRY:
        ...
        if normalized_number.startswith(default_country_code_str):
            ...
            _, potential_national_number, _ = _maybe_strip_national_prefix_carrier_code(
                potential_national_number, metadata
            )
            ...
            # instead.
            if (
                not _match_national_number(full_number, general_desc, False)
                and _match_national_number(
                    potential_national_number, general_desc, False
                )
            ) or (
...
def _maybe_strip_i18n_prefix_and_normalize(number, possible_idd_prefix):
    ...
    if m:
        ...
        # "+" sign at the start.
        return (CountryCodeSource.FROM_NUMBER_WITH_PLUS_SIGN, _normalize(number))
...
def _maybe_strip_national_prefix_carrier_code(number, metadata):
    ...
    if (
        len(number) == 0
        or possible_national_prefix is None
        or len(possible_national_prefix) == 0
    ):
    ...
    if prefix_match:
        ...
        if (
            transform_rule is None
            or len(transform_rule) == 0
            or prefix_match.groups()[num_groups - 1] is None
        ):
            # If the original number was viable, and the resultant number is not, we return.
            # Check that the resultant number is viable. If not, return.
            national_number_match = _match_national_number(
                number[prefix_match.end() :], general_desc, False
            )
            if is_viable_original_number and not national_number_match:
                return (U_EMPTY_STRING, number, False)

            if num_groups > 0 and prefix_match.groups(num_groups) is not None:
            ...
            national_number_match = _match_national_number(
                transformed_number, general_desc, False
            )
            if is_viable_original_number and not national_number_match:
                return ("", number, False)
            if num_groups > 1:
...
def _set_italian_leading_zeros_for_phone_number(national_number, numobj):
    ...
    if len(national_number) > 1 and national_number[0] == U_ZERO:
        ...
        # a leading zero.
        while (
            number_of_leading_zeros < len(national_number) - 1
            and national_number[number_of_leading_zeros] == U_ZERO
        ):
            number_of_leading_zeros += 1
...
def parse(number, region=None, keep_raw_input=False, numobj=None, _check_region=True):
    ...
    country_code = 0
    try:
        country_code, normalized_national_number = _maybe_extract_country_code(
            national_number, metadata, keep_raw_input, numobj
        )
    except NumberParseException:
        _, e, _ = sys.exc_info()
        matchobj = _PLUS_CHARS_PATTERN.match(national_number)
        if (
            e.error_type == NumberParseException.INVALID_COUNTRY_CODE
            and matchobj is not None
        ):
            # Strip the plus-char, and try again.
            country_code, normalized_national_number = _maybe_extract_country_code(
                national_number[matchobj.end() :], metadata, keep_raw_input, numobj
            )
            if country_code == 0:
                raise NumberParseException(
                    NumberParseException.INVALID_COUNTRY_CODE,
    ...
    if metadata is not None:
        ...
        carrier_code, potential_national_number, _ = (
...
def _is_phone_context_valid(phone_context):
    ...
    return fullmatch(_RFC3966_GLOBAL_NUMBER_DIGITS_PATTERN, phone_context) or fullmatch(
        _RFC3966_DOMAINNAME_PATTERN, phone_context
    )
...
def _build_national_number_for_parsing(number):
    ...
    if not _is_phone_context_valid(phone_context):
        raise NumberParseException(
            NumberParseException.NOT_A_NUMBER, "The phone-context value is invalid"
        )
    if phone_context is not None:
        ...
        index_of_national_number = (
            (index_of_rfc3996_prefix + len(_RFC3966_PREFIX))
            if (index_of_rfc3996_prefix >= 0)
            else 0
        )
...
def _is_number_match_OO(numobj1_in, numobj2_in):
    ...
    if (
        numobj1.extension is not None
        and numobj2.extension is not None
        and numobj1.extension != numobj2.extension
    ):
    ...
    if country_code1 != 0 and country_code2 != 0:
        ...
        elif country_code1 == country_code2 and _is_national_number_suffix_of_other(
            numobj1, numobj2
        ):
...
def _match_national_number(number, number_desc, allow_prefix_match):
    ...
    if (
        number_desc is None
        or number_desc.national_number_pattern is None
        or len(number_desc.national_number_pattern) == 0
    ):
        return False
    return _match(
        number, re.compile(number_desc.national_number_pattern), allow_prefix_match
    )
